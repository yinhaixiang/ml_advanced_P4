...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing == False:
            # self.epsilon = math.exp(-0.01 * self.t_num)
            # self.epsilon = math.cos(self.t_num * 0.1)
            self.epsilon = math.cos(self.t_num * 0.0015)
            self.alpha = 0.5
    ...
    def choose_action(self, state):
        ...
        if self.learning == True:
            if random.random() > self.epsilon:
                ...
                max_keys = [key for key, value in self.Q[state].items() if value == maxQ]
